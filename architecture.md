...
## Ecosystem Constraints (ECO-* Binding)

These constraints are binding across the MoneyShyft / NeighborRoute ecosystem and supersede application-specific implementation choices unless an explicit exception is documented below.
...
Tenant lookup MAY resolve against application databases (e.g., organizations, households) but MUST NOT introduce a separate tenant authority.
...
Crisis Mode MAY be served as static content under /crisis/* on the same origin to preserve:
- offline capability,
- safety guarantees,
- and same-origin security assumptions.

This exception applies only to Crisis Mode.
...
### ECO-3A: MoneyShyft Authentication Migration Exception

MoneyShyft is granted a temporary exception to ECO-3.
- MoneyShyft MAY use cookie-based authentication (HttpOnly cookies + CSRF protection).
- This exception exists to preserve delivery velocity and implementation stability.

Migration Requirement:  
MoneyShyft MUST migrate to the ecosystem authentication model in a future major version.
- The migration MUST be explicit and versioned.
...
HTTP error codes (4xx, 5xx) are reserved exclusively for:
...
Content Security Policy MUST be injected at the ingress / edge layer.
- Applications MUST NOT set or modify CSP headers.
- CSP violations are infrastructure concerns, not application logic.
...
- Crisis Mode must be deployable independently (isolated static deployment)
  - **Security Boundary:** Crisis Mode has no authentication, no required authentication, no dependency on app session state, and no required API calls for core functionality
    ...
    - **Deployment:** Crisis Mode is served under `/crisis/*` on the same origin as SPA/API (exception allowed by ECO-2)
      - Served as static files under `/crisis/*` path
      - Unauthenticated
      - Ignores cookies
...
- System must support horizontal scaling without architectural redesign
  - **Requirement:** Stateless API design (cookie-based JWT with CSRF protection under ECO-3A exception; migration required)
