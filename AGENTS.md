...
- TypeScript is used across backend and frontend; Vue SFCs live in `apps/app/src/`.
- Indentation is 2 spaces in TS/Vue files.
- Backend routes follow `apps/api/src/routes/api/v1/<resource>.ts`; validators live in `apps/api/src/validators/` with `<resource>.validators.ts`.
...
## Configuration Tips
- Backend env lives in `apps/api/.env` (gitignored). Frontend uses Vite env via `import.meta.env`.
- Do not commit secrets; copy `docker-compose.example.yml` when setting up locally.

## Production Context (MoneyShyft)
- Prod deploy uses Docker Compose with `node` built from `apps/api/Dockerfile` and Postgres in Docker; nginx is host-managed and serves `apps/app/dist`.
- Prod `docker-compose.yml` uses `NODE_ENV=production` and `apps/api/.env` via `env_file`.
- Prod nginx config proxies `/api/` to `127.0.0.1:3000` and serves the Vue SPA from `/home/jeremiahotis/projects/moneyshyft/apps/app/dist`.
- Prod build runs `npm run build` inside `apps/api/Dockerfile` and must succeed with TypeScript strictness.
