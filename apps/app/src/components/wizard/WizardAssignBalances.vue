...
function getBudgetAmount(categoryName: string): number {
  ...
  if (categoryName === 'Minimum Debt Payments') {
    const ccPayments = answers.credit_card_debts?.reduce((sum, payment) => sum + payment.minimum_payment, 0) || 0
    const otherPayments = answers.other_debts?.reduce((sum, payment) => sum + payment.minimum_payment, 0) || 0
