...
const totalDebtPayments = computed(() => {
  const ccPayments = props.answers.credit_card_debts?.reduce((sum, payment) => sum + payment.minimum_payment, 0) || 0;
  const otherPayments = props.answers.other_debts?.reduce((sum, payment) => sum + payment.minimum_payment, 0) || 0;
