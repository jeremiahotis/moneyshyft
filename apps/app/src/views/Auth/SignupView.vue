...
async function handleSignup() {
  ...
  try {
    ...
    if (axios.isAxiosError<{ details?: Array<{ message: string }> }>(error) && error.response?.data?.details) {
      validationError.value = error.response.data.details.map((detail) => detail.message).join(', ');
    } else {
      validationError.value = getApiErrorMessage(error, 'Signup failed');
    }
  }
}
