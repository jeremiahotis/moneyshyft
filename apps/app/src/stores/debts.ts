...
export const useDebtsStore = defineStore('debts', () => {
  ...
  async function fetchDebts(): Promise<void> {
    ...
    try {
      ...
      error.value = getApiErrorMessage(err, 'Failed to fetch debts');
      throw err;
    } finally {
  ...
  async function createDebt(data: CreateDebtData): Promise<Debt> {
    ...
    try {
      ...
      error.value = getApiErrorMessage(err, 'Failed to create debt');
      throw err;
    } finally {
      isLoading.value = false;
    }
  ...
  async function updateDebt(id: string, data: UpdateDebtData): Promise<Debt> {
    ...
    try {
      ...
      error.value = getApiErrorMessage(err, 'Failed to update debt');
      throw err;
    } finally {
      isLoading.value = false;
    }
  ...
  async function deleteDebt(id: string): Promise<void> {
    ...
    try {
      ...
      error.value = getApiErrorMessage(err, 'Failed to delete debt');
      throw err;
    } finally {
      isLoading.value = false;
    }
  ...
  async function addPayment(debtId: string, data: AddDebtPaymentData): Promise<Debt> {
    ...
    try {
      ...
      error.value = getApiErrorMessage(err, 'Failed to add payment');
      throw err;
    } finally {
      isLoading.value = false;
    }
  ...
  async function getPaymentHistory(debtId: string): Promise<DebtPayment[]> {
    try {
      ...
      error.value = getApiErrorMessage(err, 'Failed to fetch payment history');
      throw err;
    }
  }
  ...
  async function calculatePayoffStrategy(monthlyBudget: number): Promise<StrategyComparison> {
    ...
    try {
      ...
      error.value = getApiErrorMessage(err, 'Failed to calculate payoff strategy');
      throw err;
    } finally {
      isLoading.value = false;
    }
  ...
  async function commitPaymentPlan(data: {
    ...
    try {
      ...
      error.value = getApiErrorMessage(err, 'Failed to commit payment plan');
      throw err;
    } finally {
      isLoading.value = false;
    }
  ...
  async function getActivePaymentPlan(): Promise<unknown> {
    isLoading.value = true;
    error.value = null;
    try {
      ...
      error.value = getApiErrorMessage(err, 'Failed to fetch active payment plan');
      throw err;
    } finally {
      isLoading.value = false;
    }
