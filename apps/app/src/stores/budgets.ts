...
export const useBudgetsStore = defineStore('budgets', () => {
  ...
  async function fetchBudgetSummary(month: string): Promise<void> {
    ...
    try {
      ...
      error.value = getApiErrorMessage(err, 'Failed to fetch budget summary');
      throw err;
    } finally {
  ...
  async function fetchAllocations(month: string): Promise<void> {
    ...
    try {
      ...
      error.value = getApiErrorMessage(err, 'Failed to fetch allocations');
      throw err;
    } finally {
      isLoading.value = false;
    }
  ...
  async function setAllocation(
    ...
    try {
      ...
      error.value = getApiErrorMessage(err, 'Failed to set allocation');
      throw err;
    } finally {
      isLoading.value = false;
    }
