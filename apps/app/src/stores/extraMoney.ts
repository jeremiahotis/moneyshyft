...
export const useExtraMoneyStore = defineStore('extraMoney', () => {
  ...
  async function fetchAllEntries(status?: 'pending' | 'assigned' | 'ignored'): Promise<void> {
    ...
    try {
      ...
      error.value = getApiErrorMessage(err, 'Failed to fetch extra money entries');
      throw err;
    } finally {
  ...
  async function fetchPendingEntries(): Promise<void> {
    ...
    try {
      ...
      error.value = getApiErrorMessage(err, 'Failed to fetch pending entries');
      throw err;
    } finally {
      isLoading.value = false;
    }
  ...
  async function createEntry(data: CreateExtraMoneyData): Promise<ExtraMoneyWithAssignments> {
    ...
    try {
      ...
      error.value = getApiErrorMessage(err, 'Failed to create entry');
      throw err;
    } finally {
      isLoading.value = false;
    }
  ...
  async function assignToCategories(
    ...
    try {
      ...
      error.value = getApiErrorMessage(err, 'Failed to assign extra money');
      throw err;
    } finally {
      isLoading.value = false;
    }
  ...
  async function ignoreEntry(entryId: string): Promise<void> {
    ...
    try {
      ...
      error.value = getApiErrorMessage(err, 'Failed to ignore entry');
      throw err;
    } finally {
      isLoading.value = false;
    }
  ...
  async function deleteEntry(entryId: string): Promise<void> {
    ...
    try {
      ...
      error.value = getApiErrorMessage(err, 'Failed to delete entry');
      throw err;
    } finally {
      isLoading.value = false;
    }
  ...
  async function runDetectionScan(): Promise<number> {
    ...
    try {
      ...
      error.value = getApiErrorMessage(err, 'Failed to run detection scan');
      throw err;
    } finally {
      isLoading.value = false;
    }
